...
export async function generateMetadata({ params }: LocationPageProps): Promise<Metadata> {
  ...
  const metadataBase = new URL('https://frankandsonsexterminators.com');
  ...
  switch (params.service) {
    case 'pest-control':
      serviceContent = {
        title: `${city.name} ${stateData.abbreviation} Pest Control and Exterminators`,
        description: `Pest control services in ${city.name}, ${stateData.abbreviation}. For insects, rodents, bed bugs, fleas, ants, roaches, spiders, yellow jackets, wasps, mosquitoes, flies, etc. Call today!`
      };
      break;
    default:
      serviceContent = {
        title: `${city.name} ${stateData.abbreviation} ${service.name}`,
        description: `Professional ${service.name.toLowerCase()} services in ${city.name}, ${stateData.abbreviation}. Call 1-833-581-1133 today!`
      };
  }
  ...
  return {
    metadataBase,
    title: serviceContent.title || service.name,
    ...
    keywords: `${service.name} ${city.name}, pest control ${city.name}, pest control services ${stateData.abbreviation}, pest extermination ${city.name}, residential pest control ${city.name}, commercial pest control ${city.name}, pest control expert ${city.name}, insect control ${city.name}, rodent control ${city.name}`,
  };
}
...
export default async function LocationPage({ params }: LocationPageProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-blue-50/40">
      <SchemaOrg
        name={`${serviceData.name} in ${cityData.name}, ${stateData.abbreviation}`}
        description={`Professional ${serviceData.name} services in ${cityData.name}, ${stateData.abbreviation}. Expert ${serviceData.name} by IQ Appliances.`}
        city={cityData}
